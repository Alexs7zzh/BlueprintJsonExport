...
int32 UBlueprintJsonExportCommandlet::Main(const FString& Params)
{
    ...
    for (const FAssetData& Asset : AssetsToExport)
    {
        ...
        {
            ++SuccessCount;
            UE_LOG(LogBlueprintJsonExport, Display, TEXT("Exported %s -> %s"),
                *Asset.GetObjectPathString(), *Result.OutputFilePath);
        }
        else
        {
            ++FailureCount;
            UE_LOG(LogBlueprintJsonExport, Error, TEXT("Failed to export %s: %s"),
                *Asset.GetObjectPathString(), *Result.Error);
        }
    }

    UE_LOG(LogBlueprintJsonExport, Display, TEXT("BlueprintJsonExport completed. Success: %d, Failed: %d"),
